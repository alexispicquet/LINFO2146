  ...
  const linkaddr_t *src, const linkaddr_t *dest)
{
  if(len == sizeof(packet_t)) {
    ...
    if(pkt.status==PARENT) {
        add_acker(kids, init_pkt2(&pkt), *src);
    }
    LOG_INFO_LLADDR(src);
    LOG_INFO_(" -> %u, status %u, %u %u action, #nie %u, #kids %u",node_id, pkt.status, res,res2, size_list(neighbours), size_list(kids));
    LOG_INFO_("\n");
  }
}
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(nullnet_example_process, ev, data)
{
  ...
  while(1) {
    ...
    // broadcast if no neighbours
    if(size_list(neighbours)==0) {
